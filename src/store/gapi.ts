...
export const useGAPIStore = defineStore("gapi", () => {
  ...
  function setDateChangeInProgress(inProgress: boolean) {
    ...
    console.log(
      `📅 Date change operation ${inProgress ? "started" : "completed"}`
    );
  }
  ...
  async function checkExistingAuth() {
    ...
    if (storedToken) {
      ...
      try {
        ...
        console.log(
          "🎉 Successfully restored authentication from stored token"
        );
        return;
      } catch (error) {
  ...
  async function testTokenValidity(): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Token info request failed: ${response.status} ${response.statusText}`
        );
      }
      ...
      if (spreadsheetId) {
        ...
        if (!sheetResponse.ok) {
          throw new Error(
            `Spreadsheet access failed: ${sheetResponse.status} ${sheetResponse.statusText}`
          );
        }
    ...
    google.accounts.id.initialize({
      ...
      callback: async (
        ...
        try {
          ...
          console.log(
            "🔑 Automatically requesting API access after One Tap sign-in"
          );
          if (tokenClient) {
  ...
  async function loadSettings(): Promise<void> {
    ...
    if (
      dayStartFromSheet &&
      typeof dayStartFromSheet === "string" &&
      dayStartFromSheet.trim()
    ) {
  ...
  async function fetchSheetValues(
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch sheet values: ${response.status} ${response.statusText}`
      );
    }
  ...
  async function updateSheetValues(
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to update sheet values: ${response.status} ${response.statusText}`
      );
    }
  }
  ...
  async function batchUpdateSheetValues(
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to batch update sheet values: ${response.status} ${response.statusText}`
      );
    }
  }
  ...
  function verifyReadiness() {
    ...
    if (!isSignedIn.value || !route.params.id || !accessToken.value) {
      throw new Error(
        `cannot fetch data from google , spid: ${
          route.params.id
        }, isSignedIn: ${isSignedIn.value}, hasToken: ${!!accessToken.value}`
      );
    }
  }
  ...
  async function updateSignInStatus(signedIn: boolean) {
    ...
    console.log(
      `🔑 updateSignInStatus called: ${signedIn ? "signed in" : "signed out"}`
    );
    ...
    if (signedIn) {
      ...
      const hasRealAssignmentData =
        positions.some((p) =>
          p.shifts.some(
            (s) =>
              s.soldierIds && s.soldierIds.some((id) => id && id.trim() !== "")
          )
        ) ||
        Array.from(historicalPositions.values()).some((datePositions) =>
          datePositions.some((p) =>
            p.shifts.some(
              (s) =>
                s.soldierIds &&
      ...
      const shouldSkipTemplateLoad =
        ...
        isDateChangeInProgress;

      console.log(
        `🔍 Template load decision: skip=${shouldSkipTemplateLoad} (current: ${hasCurrentPositions}, historical: ${hasHistoricalData}, loaded dates: ${hasLoadedDates}, real data: ${hasRealAssignmentData}, date change in progress: ${isDateChangeInProgress})`
      );

      if (!shouldSkipTemplateLoad) {
        console.log(
          `📋 No existing positions or historical data, loading from template initially`
        );
        await loadPositionsForDate(); // This will load from template initially
      } else {
        console.log(
          `✅ Positions or historical data already exists (current: ${positions.length}, historical: ${historicalPositions.size}, loaded dates: ${loadedDates.size}, has real data: ${hasRealAssignmentData}), skipping template load`
        );
      }
  ...
  async function loadSoldiers(): Promise<void> {
    ...
    console.log(
      `👥 Processed ${processedSoldiers.length} soldiers from ${soldiersArr.length} raw rows`
    );
    console.log(
      `👥 Soldier IDs loaded: ${processedSoldiers
        .slice(0, 10)
        .map((s) => s.id)
        .join(", ")}${processedSoldiers.length > 10 ? "..." : ""}`
    );
  ...
  async function getSheetIdByName(sheetName: string): Promise<number | null> {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to get spreadsheet info: ${response.status} ${response.statusText}`
        );
      }
  ...
  async function duplicateSheet(
    ...
    try {
      ...
      const sourceSheet = sheets.find(
        (s: any) => s.properties?.title === sourceSheetName
      );
      if (!sourceSheet) {
      ...
      console.log(
        `📊 Current spreadsheet has ${currentSheetCount} sheets, adding new sheet at the end`
      );
      ...
      if (!duplicateResponse.ok) {
        throw new Error(
          `Failed to duplicate sheet: ${duplicateResponse.status} ${duplicateResponse.statusText}`
        );
      }

      console.log(
        `✅ Sheet duplicated: "${sourceSheetName}" → "${newSheetName}" (added at position ${
          currentSheetCount + 1
        })`
      );
    } catch (error) {
  ...
  function getCurrentSheetName(scheduleDate?: Date): string {
    return scheduleDate
      ? generateSheetNameFromDate(scheduleDate)
      : SHEETS.POSITIONS;
  }
      ...
      for (let i = 0; i < row.length; ++i) {
        switch (row[i]) {
          ...
          case TITLES.ASSIGNMENT:
            ...
            console.log(
              `🔍 Loading assignment ${assignmentIndex}: "${assignmentValue}" for position ${state.position.name}`
            );
            console.log(
              `📊 Position has ${state.position.shifts.length} shifts:`
            );
            state.position.shifts.forEach((shift, idx) => {
              console.log(
                `   Shift ${idx} (${shift.id}): ${shift.assignmentDefs.length} slots`
              );
            });
            ...
            console.log(
              `🔢 Calculating placement for assignment index ${assignmentIndex}:`
            );

            for (
              let shiftIdx = 0;
              shiftIdx < state.position.shifts.length;
              shiftIdx++
            ) {
              const shift = state.position.shifts[shiftIdx];
              const shiftSlots = shift.assignmentDefs.length;
              console.log(
                `   Checking shift ${shiftIdx} (${shift.id}): currentIndex=${currentIndex}, shiftSlots=${shiftSlots}`
              );
              if (currentIndex < shiftSlots) {
                targetShift = shift;
                targetSpotIndex = currentIndex;
                console.log(
                  `   ✅ Found target: shift ${shiftIdx} (${shift.id}), spot ${targetSpotIndex}`
                );
                break;
              }
              currentIndex -= shiftSlots;
              console.log(
                `   ⏭️ Moving to next shift, remaining currentIndex=${currentIndex}`
              );
            }

            if (targetShift && targetSpotIndex >= 0) {
              console.log(
                `📍 Placing "${assignmentValue}" in shift ${targetShift.id} at spot ${targetSpotIndex}`
              );
              ...
              console.log(
                `✅ Result: shift ${
                  targetShift.id
                } soldierIds: [${targetShift.soldierIds.join(", ")}]`
              );
  ...
  async function loadPositionsForDate(scheduleDate?: Date): Promise<void> {
    ...
    try {
      ...
      if (!sheetExists && scheduleDate) {
        console.log(
          `📋 Creating new sheet from template: "${targetSheetName}"`
        );
   ...
   */
  async function loadPositionsForSingleDate(
    date: Date
  ): Promise<PositionDto[]> {
    if (!isSignedIn.value) return [];
    ...
    console.log(
      `📅 Loading positions for date: ${dateKey} (sheet: "${targetSheetName}")`
    );
    ...
    try {
      ...
      await loadPositionsFromSheetIntoArray(
        targetSheetName,
        tempPositions,
        date
      );
      ...
      console.log(
        `✅ Loaded ${tempPositions.length} positions for date: ${dateKey}`
      );
      ...
      if (existingData && existingData.length > 0) {
        console.log(
          `✅ Using previously loaded data for date ${dateKey} (${existingData.length} positions)`
        );
        return existingData;
      }
      ...
      try {
        ...
        console.log(
          `✅ Loaded ${tempPositions.length} positions from template for date: ${dateKey}`
        );
        return tempPositions;
      } catch (fallbackError) {
        console.error(
          `❌ Even template fallback failed for date ${dateKey}:`,
          fallbackError
        );
   ...
   */
  async function loadPositionsFromSheetIntoArray(
    sheetName: string,
    targetArray: PositionDto[],
    dateContext?: Date
  ): Promise<void> {
    ...
    try {
      ...
      if (dateContext) {
        ...
        console.log(
          `📅 Set processing date context: ${format(dateContext, "yyyy-MM-dd")}`
        );
      }
   ...
   */
  async function loadPositionsForDateRange(
    currentDate: Date,
    pastDays: number = 3
  ): Promise<void> {
    if (!isSignedIn.value) return;
    ...
    console.log(
      `📅 Loading positions for date range: ${pastDays} days before ${getDateKey(
        currentDate
      )}`
    );
    ...
    console.log(
      `📋 Dates to load: ${datesToLoad.map((d) => getDateKey(d)).join(", ")}`
    );

    // Load all dates in parallel
    await Promise.all(
      datesToLoad.map((date) => loadPositionsForSingleDate(date))
    );

    // Update main positions array with current date positions
    const currentDatePositions =
      historicalPositions.get(getDateKey(currentDate)) || [];
    positions.splice(0, positions.length, ...currentDatePositions);

    console.log(
      `✅ Date range loading complete. Main positions updated with ${positions.length} positions`
    );
  }
   ...
   */
  async function loadPositionsIncremental(
    newCurrentDate: Date,
    pastDays: number = 3
  ): Promise<void> {
    if (!isSignedIn.value) return;
    ...
    console.log(
      `🔄 Incremental loading for date: ${getDateKey(
        newCurrentDate
      )} (${pastDays} days history)`
    );
    ...
    const missingDates = requiredDates.filter(
      (date) => !loadedDates.has(getDateKey(date))
    );

    if (missingDates.length === 0) {
      console.log(
        `✅ All required dates already loaded, just updating main positions`
      );
    } else {
      console.log(
        `📋 Missing dates to load: ${missingDates
          .map((d) => getDateKey(d))
          .join(", ")}`
      );
      ...
      await Promise.all(
        missingDates.map((date) => loadPositionsForSingleDate(date))
      );
    }

    // Update main positions array with current date positions
    const currentDatePositions =
      historicalPositions.get(getDateKey(newCurrentDate)) || [];

    // Debug: Check if current date positions have soldier assignments
    console.log(
      `🔍 Current date positions summary:`,
      currentDatePositions.map((pos) => ({
        name: pos.name,
        shifts: pos.shifts.map((shift) => ({
          id: shift.id,
          soldierIds: shift.soldierIds || [],
          hasAssignments: (shift.soldierIds || []).some(
            (id) => id && id.trim() !== ""
          ),
          assignmentDetails: (shift.soldierIds || []).filter(
            (id) => id && id.trim() !== ""
          ),
        })),
      }))
    ...
    console.log(
      `👥 Soldiers assigned in current date (${getDateKey(
        newCurrentDate
      )}) positions:`,
      Array.from(currentDateAssignedSoldiers)
    );

    // Set processing date context for current date before updating positions
    currentProcessingDate.value = newCurrentDate;
    console.log(
      `📅 Set processing date context for current date: ${format(
        newCurrentDate,
        "yyyy-MM-dd"
      )}`
    );
    ...
    console.log(
      `✅ Incremental loading complete. Main positions updated with ${positions.length} positions`
    );
  }
  ...
  return {
    ...
    // Expose access token for external use
